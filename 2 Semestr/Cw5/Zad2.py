...
def add_zawodnik():
    ...
    sesja.add(Zawodnicy(numer_zawodnika=numer_zawodnika_new,imie_zawodnika=imie_zawodnika_new,nazwisko_zawodnika=nazwisko_zawodnika_new))
    read_zawodnicy()
...
def delete_zawodnik():
    ...
    nazwisko = str(input())
    sesja.query(Zawodnicy).filter(Zawodnicy.imie_zawodnika == imie and Zawodnicy.nazwisko_zawodnika == nazwisko).delete(synchronize_session="evaluate")
    sesja.commit()
    read_zawodnicy()
...
def update_zawodnika():
    ...
    updating_criteria = str(input("Podaj co chcesz zmienić?\nDla odpowiedznich pól wpisz:\n Numer zawodnika\n Imię\n Nazwisko\n"))
    
    if updating_criteria == "Numer zawodnika":
        wartosc = int(input("Na wartość :\n"))
        sesja.query(Zawodnicy).filter(Zawodnicy.numer_zawodnika==ajdi).update({Zawodnicy.numer_zawodnika:wartosc},synchronize_session ="evaluate")

    elif updating_criteria == "Imię":
        wartosc = str(input("Na wartość :\n"))
        sesja.query(Zawodnicy).filter(Zawodnicy.imie_zawodnika==ajdi).update({Zawodnicy.imie_zawodnika:wartosc},synchronize_session ="evaluate")

    elif updating_criteria == "Nazwisko":
        wartosc = str(input("Na wartość :\n"))
        sesja.query(Zawodnicy).filter(Zawodnicy.nazwisko_zawodnika==ajdi).update({Zawodnicy.nazwisko_zawodnika:wartosc},synchronize_session ="evaluate")
    else:
...
def read_mecze():
    for mecze in sesja.query(Mecze).join(Zawodnicy).all():
        print(mecze.numer_meczu,mecze.przeciwko,mecze.najlepszy_zawodnik,mecze.zawodnicy.imie_zawodnika,mecze.zawodnicy.nazwisko_zawodnika)
    print()
...
def update_mecz():
    ...
    if updating_criteria == "Numer meczu":
        wartosc = int(input("Na wartość :\n"))
        sesja.query(Mecze).filter(Mecze.numer_meczu==ajdi).update({Mecze.numer_meczu:wartosc},synchronize_session ="evaluate")

    elif updating_criteria == "Przeciwko":
        wartosc = str(input("Na wartość :\n"))
        sesja.query(Mecze).filter(Mecze.przeciwko==ajdi).update({Mecze.przeciwko:wartosc},synchronize_session ="evaluate")

    elif updating_criteria == "Najlepszy zawodnik":
        wartosc = str(input("Na wartość :\n"))
        sesja.query(Mecze).filter(Mecze.najlepszy_zawodnik==ajdi).update({Mecze.najlepszy_zawodnik:wartosc},synchronize_session ="evaluate")
    else:
        print("Nie ma takiego kryterium")
        pass
    read_mecze()
